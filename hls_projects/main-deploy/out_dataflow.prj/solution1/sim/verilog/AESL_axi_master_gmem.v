...
//------------------------Parameter----------------------
`define TV_IN_gmem "../tv/cdatafile/c.test.autotvin_gmem.dat"
`define TV_OUT_gmem "../tv/rtldatafile/rtl.test.autotvout_gmem.dat"
...
parameter gmem_mem_depth = 32'd 166400;
...
output [32 - 1 : 0] TRAN_gmem_input_image;
output [32 - 1 : 0] TRAN_gmem_result;
input ready;
input done;
...
assign    TRAN_gmem_input_image = input_image;
assign    TRAN_gmem_result = result;
...
initial begin : initialize_offset
  ...
  input_image <= 0 * DATA_byte_num;
  result <= 153600 * DATA_byte_num;
end
    ...
    while (1) begin
        if (FIFO_AW_req_empty === 1 || BRESP_counter == 32) begin
            ...
            if (FIFO_AW_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/work/shared/users/meng/sjz38/tmp/my_ultranet/hls_projects/main-deploy/out_dataflow.prj/solution1/sim/verilog/AESL_axi_master_gmem.v: Write request address %d exceed AXI master gmem array depth: %d",FIFO_AW_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
    ...
    while (1) begin
        ...
        if (FIFO_AR_req_empty) begin
            ...
            if (FIFO_AR_req_ADDR_tmp/data_byte_size > gmem_mem_depth) begin
                $display ("/work/shared/users/meng/sjz38/tmp/my_ultranet/hls_projects/main-deploy/out_dataflow.prj/solution1/sim/verilog/AESL_axi_master_gmem.v: Read request address %d exceed AXI master gmem array depth: %d",FIFO_AR_req_ADDR_tmp/data_byte_size, gmem_mem_depth); 
                $finish;
            end
...
initial begin : read_file_process 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 153600; i < 166400 ; i = i + 1) begin 
          token = read_token(fp);
          ret = $sscanf(token, "0x%x", token_tmp); 
          if (factor == 4) begin
              if (i%factor == 0) begin
