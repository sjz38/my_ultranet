...
# batch normalization
def batchnorm2d_old(data, gamma, beta, moving_mean, moving_var, axis = 1, epsilon=10**-7, name="batch_norm", inter_dtype=hcl.Fixed(16,8), out_dtype=hcl.Fixed(16,8)):
...
# batch normalization
def batchnorm2d(data, a, bias, axis=1, name="batch_norm"):
    def get_axis(axis, *indices):
        indices = list(indices[0])
        return (indices[axis],)
    print("data shape:\n", data.shape)
    out = hcl.compute(data.shape, lambda *x : a[get_axis(axis, x)] * data[x] - bias[get_axis(axis, x)], dtype=data.dtype, name=name)
    print(out.shape)
    return out